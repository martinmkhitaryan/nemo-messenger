...
`crates/nemo-wire` (MIT) encodes version-1 cards, signatures, envelopes, and HPKE. `crates/nemo-server` (MIT) is the blind mailbox, group-stream, and local HTTP engine. `crates/nemo-core` (AGPL-3.0-only) is the client identity, PQXDH/Double Ratchet, and MLS crate. `nemo-wire` and `nemo-server` MUST NOT depend on `libsignal` or `nemo-core`.
