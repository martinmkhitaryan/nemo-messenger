...
Emulator instrumented tests still need a running `nemo-server` on `0.0.0.0:18787` (see below). The Windows job is `cargo build -p nemo-ffi` and only runs on Windows or in GitHub. Linux `cargo build -p nemo-ffi` does not catch this. On a Windows machine, use an “x64 Native Tools” prompt so `cl` is `CC`/`CXX` (sqlcipher’s OpenSSL nmake is `VC-WIN64A`; GNU `clang` rejects `/Zi`). Keep Strawberry Perl’s `c++` off `PATH`, but leave Strawberry `perl` on `PATH` (Git’s MSYS perl cannot run `Configure`). Use a short `CARGO_TARGET_DIR` (`MAX_PATH`). MSVC needs C++20 for APM designated initializers; GitHub uses a `cl.exe` shim that rewrites `/std:c++17` to `/std:c++20` and quotes the real compiler path under `Program Files` (Cargo will not run a `meson.cmd` wrapper):
